...
There are two ways to create alerts. The first is through the command line and is mainly useful for creating alerts one at a time this is further explained in the [Creating Alerts](#creating-alerts) section. The second is by defining all of the parameters that the user would input in the command line through a JSON expression that is stored in the `alert_config.json` file. An example of what the `alert_config.json` file contains is shown below for the "Alice 50 K Temperature" alert.
...
To create an alert after defining how to retrieve the measurement from Influx DB within the `alerts_config.json` file the user simply has to input the state, threshold and title in a configuration file as shown below; notice how below the title follows the naming convention outlined above. This file must end in "config.json" and be located in the `~/IoT-Server/Alerts-API` for the program to recognize it. 
...
Once this configuration file is created all that must be done is to enable this configuration; this is further explained in the next section. 
