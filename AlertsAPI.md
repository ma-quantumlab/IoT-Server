...
- [Setting Alert Configurations](#setting-alert-configurations)
...
Alerts will be visible directly on any timeseries panel that queries the same Influx DB data as the alert; as shown below for the following configuration of alerts:
...
The Grafana timeseries panel shades in "red"/"dark-red" or "orange"/"dark-orange" regions for the different types of alerts.
...
Alerts are named in a way that allows for no repeats and allows both the program and user to identify which type of alert it is. This means that the alert name is what the program uses to create the different type of alerts and thus if an alert is created that does not follow this naming convention then it will result in incorrect alerts. Luckly this system is very simple to understand. 

To start take the name "Alice 50 K Temperature Upper Critical Alert", there are two parts to every name. First is the unique identifier that the program uses to tell grafana which Influx DB feild to retrieve, in this case that is "Alice 50 K Temperature". 
...
- Upper Critical Alert
- Upper Warning Alert
- Lower Warning Alert 
- Lower Critical Alert

The "Upper" or "Lower" key work tells the program weither to trigger when the value is greater or less then the threshold respectivly. The "Critical" or "Warning" key word tells the program to create a "Critical" or "Warning" alert. Critical and Warning alerts differ in both the color which they are displayed on in the dashboard panel (Ciritical alerts are red while Warning alerts are orange) and also which notification policy to trigger. 

When creating alerts 

## Setting Alert Configurations
...
If the user is intrested in creating a new configuration file 
...
Following this the program will prompt the user to do the following: Enter in a name, this name should simply be a uniuque identifier of which measurement feild is being used to query the Influx DB data. The name should not include identifiers baised on which type of alert it is, i.e. if it is a critical alert or warning alert or if it is an upper bounded or lower bounded alert these will autonamtically be added by the program. For example in this case the name "Alice 50 K Temperature" was used and the final name of the alert that will be "Alice 50 K Temperature Upper Critical Alert".

The program will also ask the user to specifiy if the alert should fire if no data is coming in; this should be set to "Alerts" only in the sinario in which no data coming in should trigger the alert. 
...
If the user choses to configure an alert-dashboard pair then they will have to first chose a database, then a respectice panel in which to display the alert on. Note that this is only for "timeseries" panels and the panels configured to any other setting do not have the option to display alerts. 
